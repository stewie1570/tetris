...
class SinglePlayerGame extends Component {
  constructor() {
    ...
    this.userNameProvider = async () =>
      await new AggregateUserNameProvider({
        underlyingProviders: [
          () => this.state.username,
          async () => {
          ...
          <div className="well app">
            <p>
              {`Score: ${this.state.game.score}` +
                (this.state.game.oldScore
                  ? ` (Previous: ${this.state.game.oldScore})`
                  : "")}
            </p>
            <div className="game">
              <TetrisGame
                  ...
                  <table className="table">
                    ...
                    <tbody>
                      {this.state.scoreBoard === loading ? (
                        ...
                        this.state.scoreBoard.map((userScore, index) => (
                          <tr key={index}>
            ...
            <div className="controls">
              <CommandButton
                ...
                onClick={({ target }) =>
                  target.blur() ||
                  this.controller.pause({ isPaused: this.state.paused })
                }
              >
                <span
              ...
              <div>
                ...
                <CommandButton
                  ...
                  onClick={({ target }) =>
                    target.blur() ||
                    this.setState(({ game }) => ({
                      game: { ...game, mobile: !game.mobile },
                    }))
                  }
                  disabled={this.state.paused}
                >
                  {this.state.game.mobile
                    ? "No Mobile Controls"
                    : "Mobile Controls"}
                </CommandButton>
              </div>
