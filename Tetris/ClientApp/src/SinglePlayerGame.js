...
const useBehaviorSharingInGameContext = (behaviors) => {
  const { setGame } = useContext(SinglePlayerGameContext);
  ...
  useEffect(() => {
    setGame((game) => ({ ...game, behaviors: behaviorsRef.current }));
  }, []);
};
...
export const SinglePlayerGame = ({
  ...
  const { game, setGame, username, setUsername, prompt } = useContext(
  ...
  useBehaviorSharingInGameContext({ pause });
